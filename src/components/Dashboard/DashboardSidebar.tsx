...
export function DashboardSidebar({isMenuOpen, setIsMenuOpen}: {isMenuOpen: boolean, setIsMenuOpen: (isMenuOpen: boolean) => void}) {
  const pathname = usePathname();
  const locale = useLocale();

  const sidebarSections = (hidden = false) => (
    <aside className={`${hidden ? "hidden w-[270px]" : " w-full"}  flex flex-col border-e border-slate-100 bg-white lg:flex h-dvh fixed top-0 start-0`}>
      <LinkTo href="/" className="flex items-center gap-3 px-6 py-6 max-w-[200px]!">
