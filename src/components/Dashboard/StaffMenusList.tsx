      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
        {menus.map((menu) => {
          ...
          return (
            <article
              key={menu.id}
              className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition-shadow hover:shadow-md dark:border-slate-700/80 dark:bg-slate-800/90"
            >
              <div className="flex items-center gap-3">
                {menu.logo ? (
                  ...
                  <span className="flex size-12 items-center justify-center rounded-xl bg-primary/10 text-lg font-semibold text-primary">
                    {name.charAt(0)}
                  </span>
                )}
                <h2 className="min-w-0 flex-1 truncate text-base font-bold text-slate-800 dark:text-slate-100">
                  {name}
                </h2>
              </div>
              ...
              <div className="mt-auto flex items-center gap-2">
                {canManageMenu && (
                  <LinkTo
                    ...
                    className="inline-flex flex-1 items-center justify-center rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-primary/90"
                  >
                    {t("menuCard.manage")}
                  </LinkTo>
                )}
                {publicUrl && (
                  <a
                    ...
                    className={`inline-flex items-center justify-center gap-1.5 rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-600 transition-colors hover:bg-slate-50 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-700/50 ${
