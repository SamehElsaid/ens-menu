...
const ActionCellRenderer = (
  params: ICellRendererParams<OnlineBooking> & { t?: (key: string) => string }
) => {
...
const AcceptRefuseCellRenderer = (
  params: ICellRendererParams<OnlineBooking> & {
    t?: (key: string) => string;
    onAccept?: (booking: OnlineBooking) => void;
    onRefuse?: (booking: OnlineBooking) => void;
  }
) => {
...
export default function OnlineBookingsPage() {
  ...
  const [selectedBooking, setSelectedBooking] = useState<OnlineBooking | null>(
    null
  );

  // Handle accept booking - open modal
  const handleAccept = useCallback((booking: OnlineBooking) => {
  ...
  const handleRefuse = useCallback((booking: OnlineBooking) => {
    setSelectedBooking(booking);
  ...
  const [allRowData] = useState<OnlineBooking[]>([
    {
      id: "1",
      date: "2024-01-15",
      parentName: "John Doe",
  ...
  // Column definitions
  const columnDefs: ColDef<OnlineBooking>[] = useMemo(
    () => [
      {
        headerName: t("bookings.actions") || "Actions",
        cellRenderer: (params: ICellRendererParams<OnlineBooking>) => (
    ...
    [t, locale, handleAccept, handleRefuse]
  );
    ...
    <CardDashBoard>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">
          {t("bookings.offline") || "Offline Bookings"}
        </h1>
        <p className="text-gray-500 mt-1">
          Manage and view all your offline reservations
        </p>
      </div>
