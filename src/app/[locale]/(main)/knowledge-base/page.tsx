...
function KnowledgeBaseInner() {
  ...
  useEffect(() => {
    if (urlId) setSelectedId(Number(urlId));
  }, [urlId]);
    ...
    >
      ...
      <aside className="w-72 shrink-0 sticky top-[72px] self-start h-[calc(100vh-72px)] flex flex-col bg-white dark:bg-[#0d1117] border-e border-slate-200 dark:border-slate-800">
        ...
        {pagination.totalPages > 1 && (
          <div className="shrink-0 px-3 py-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-2">
            <button
      ...
      <main className="flex-1 min-w-0 overflow-y-auto">
        ...
        {!detailLoading && article && (
          <>
            {/* hero header */}
            <div className="bg-white dark:bg-[#0d1117] border-b border-slate-200 dark:border-slate-800 px-6 md:px-10 py-8">
            ...
            <div
              dir={isRTL ? "rtl" : "ltr"}
              className="bg-white dark:bg-[#0d1117] px-6 md:px-10 py-6 show-editor"
            >
